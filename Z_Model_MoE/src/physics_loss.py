...
def validate_physics_constraints(predictions, features=None, return_details=False):
    ...
    # 定义BUF列索引（基于实际的TARGET_COLUMNS）
    buf_indices = {
        'inf_flu': TARGET_COLUMNS.index('Inf_Flu_BUF'),
        'fin_flu': TARGET_COLUMNS.index('Fin_Flu_BUF'),
        'inf_exp': TARGET_COLUMNS.index('Inf_Exp_BUF'),
        'fin_exp': TARGET_COLUMNS.index('Fin_Exp_BUF'),
        'inf_eff': TARGET_COLUMNS.index('Inf_Eff_BUF'),
        'fin_eff': TARGET_COLUMNS.index('Fin_Eff_BUF')
    }
    
    validation_results = {}
    
    # 1. 累积因子约束验证 (所有BUF >= 1)
    all_buf_values = predictions_original  # 所有BUF列
    cumulative_violations = (all_buf_values < MIN_BUF_VALUE).sum().item()
    validation_results['cumulative_violations'] = cumulative_violations
    validation_results['cumulative_violation_rate'] = cumulative_violations / (batch_size * len(buf_indices))
    
    # 2. 几何约束验证 (有限几何 <= 无限几何)
    inf_flu_buf = predictions_original[:, buf_indices['inf_flu']]
    fin_flu_buf = predictions_original[:, buf_indices['fin_flu']]
    inf_exp_buf = predictions_original[:, buf_indices['inf_exp']]
    fin_exp_buf = predictions_original[:, buf_indices['fin_exp']]
    inf_eff_buf = predictions_original[:, buf_indices['inf_eff']]
    fin_eff_buf = predictions_original[:, buf_indices['fin_eff']]
    
    geometry_violations = (
        (fin_flu_buf > inf_flu_buf).sum() +
        (fin_exp_buf > inf_exp_buf).sum() +
        (fin_eff_buf > inf_eff_buf).sum()
    ).item()
    
    validation_results['geometry_violations'] = geometry_violations
    validation_results['geometry_violation_rate'] = geometry_violations / (batch_size * 3)
    
    # 3. 总体约束满足率
    total_constraints = batch_size * (len(buf_indices) + 3)  # 累积约束 + 3个几何约束
    ...
    if return_details:
        validation_results['details'] = {
            'all_buf_values': all_buf_values.detach().cpu().numpy(),
            'fin_flu_vs_inf_flu': (fin_flu_buf - inf_flu_buf).detach().cpu().numpy(),
            'fin_exp_vs_inf_exp': (fin_exp_buf - inf_exp_buf).detach().cpu().numpy(),
            'fin_eff_vs_inf_eff': (fin_eff_buf - inf_eff_buf).detach().cpu().numpy()
        }
